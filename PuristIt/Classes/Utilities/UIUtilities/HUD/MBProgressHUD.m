...
#define LABELDETAILSFONTSIZE 10.0
...
- (id)initWithView:(UIView *)view withLines:(int)_lineNums {
	...
	self.lineNums = _lineNums;
	
	return [self initWithFrame:view.bounds];
}

- (id)initWithView:(UIView *)view {
	return [self initWithView:view withLines:6];
}
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
		...
		self.background = [[[UIView alloc] initWithFrame:self.bounds] autorelease];
		self.background.backgroundColor = [UIColor blackColor];
		self.background.alpha = 0.7f;
		self.background.userInteractionEnabled = NO;
		self.background.autoresizingMask =  UIViewAutoresizingFlexibleBottomMargin | 
...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
        ...
        //SR_LOG(@"label origin %f", label.frame.origin.x);
        ...
        if (nil != self.detailsLabelText) {
            ...
            self.height = self.height + (lHeight * self.lineNums) + PADDING;
			
			if ([self.labelText isEqual: @""]) {
				labelYShift = 23;
			}
			
            //SR_LOG(@"indicator origin %f", indicator.frame.origin.x);
            ...
            CGRect lFrameD = CGRectMake(floor((frame.size.width - lWidth) / 2) + xOffset,
                                        lFrame.origin.y + lFrame.size.height + PADDING - labelYShift, lWidth, lHeight * self.lineNums);
            detailsLabel.frame = lFrameD;
			
            //SR_LOG(@"detailsLabel origin %f", detailsLabel.frame.origin.x);
...
- (void)touched {
    if(delegate != nil && [delegate conformsToProtocol:@protocol(MBProgressHUDDelegate)]) {
		if([delegate respondsToSelector:@selector(hudWasTouched)]) {
			[delegate performSelector:@selector(hudWasTouched)];
		}
    }
}
